...
const gitconfigPath = '../git';

const GLOBAL_CONFIG_SOURCE = fs.readFileSync(
  join(__dirname, '../../src/__tests__/fixture/global_config'),
  'utf-8',
);
...
const LOCAL_CONFIG_SOURCE = fs.readFileSync(
  join(__dirname, '../../src/__tests__/fixture/local_config'),
  'utf-8',
);
...
test.serial('get global gitconfig', async (t) => {
  const fsMock = {
    readFile: function (path: string, _: any, cb: any) {
...
test.serial('get local gitconfig', async (t) => {
  const fsMock = {
    readFile: function (path: string, enc: any, cb: any) {
